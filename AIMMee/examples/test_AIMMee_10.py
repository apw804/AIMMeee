...
from AIMMee import Sim, AIMMeeLogger, AIMM_MME, AIMM_Scenario, AIMM_RIC, np_array_to_str
from Q_learning_generalized_01 import Q_learner

class MyScenario_OLD(AIMM_Scenario):
  def loop(self,interval=1.0,speed=2.0):
    ...
    while True:
      ...
      for ue in self.sim.UEs:
        dx=a*standard_normal(1)
        dy=a*standard_normal(1)
...
class MyScenario(AIMM_Scenario): # circle
  def loop(self,interval=1.0,speed=2.0):
    while True:
      for ue in self.sim.UEs:
        ue.xyz[0]=500.0+150.0*np.cos(1e-3*self.sim.env.now)+10*standard_normal(1)
        ue.xyz[1]=500.0+150.0*np.sin(1e-3*self.sim.env.now)+10*standard_normal(1)
      yield self.sim.wait(interval)

class RSRPLogger(AIMMeeLogger):
  def loop(self):
    while True:
...
class ThroughputLogger(AIMMeeLogger):
  def loop(self):
    alpha=0.01; beta=1.0-alpha # smoothing parameters
    ric_celledge=self.sim.ric.celledge # set in AIMM_RIC
    tp_smoothed=0.0
    while True:
...
if __name__ == '__main__':
  ...
  until=1000
  argc=len(sys.argv)
  if argc>1: until=float(sys.argv[1])
  example_n10(until)
